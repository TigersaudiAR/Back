...
### قراءة القرآن الكريم
- **مصدر رسمي موثوق**: جميع البيانات القرآنية من مجمع الملك فهد لطباعة المصحف الشريف
- عرض النص القرآني بخط **UthmanicHafs** الرسمي
- قارئ الصور (QuranReader): واجهة عصرية لعرض صفحات المصحف كصور بجودة عالية
- قارئ النص الكلاسيكي (QuranClassic): عرض نصي قابل للطباعة مع الحفاظ على التنسيق العثماني
- تنسيق مطابق لمصحف المدينة النبوية
- حفظ آخر موضع قراءة تلقائياً
- دعم التنقل بلوحة المفاتيح والحركات اللمسية
- إمكانية عرض التفسير والتلاوة الصوتية (قريباً)
...
إنشاء ملف `.env` في مجلد `front/` (يمكنك نسخ `.env.example`):
...
- `VITE_QURAN_BASE`: عنوان API مجمع الملك فهد (الافتراضي: https://qurancomplex.gov.sa/quran-dev/)
- `VITE_QURAN_API_KEY`: مفتاح API إذا كان مطلوباً (اختياري)
- `VITE_QURAN_PROXY`: عنوان Netlify Functions للوكيل (اختياري)
